...
class LLM:
    ...
    try:
        ...
        _image_analysis_prompt = Template(
            files("doc_parser").joinpath("image_analysis.j2").read_text()
        )
        _md_prompt_template = Template(
            files("doc_parser").joinpath("markdown_prompt.j2").read_text()
        )
        ...
        if self.provider == "ollama":
            ...
            if self.provider == "openai" and self.openai_config.get(
                ...
                try:
                    ...
                    raise ImportError(
                        "OpenAI is not installed. Please install it using pip install 'doc-parser[openai]'."
                    )
            ...
            try:
                ...
                raise ImportError(
                    "Gemini is not installed. Please install it using pip install 'doc-parser[gemini]'."
                )
